...
export function PluralLogo({ compact = false }: PluralLogoProps): JSX.Element {
  return (
    <div className="flex items-center gap-3">
      {compact ? (
        <svg width={28} height={28} viewBox="0 0 34 34" aria-hidden="true">
          <defs>
            <linearGradient id="pluralLogoGrad" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <path
            ...
            fill="url(#pluralLogoGrad)"
          />
        </svg>
      ) : (
        <img src={logoUrl} alt="Plural PLM" className="h-8 w-auto" />
      )}
    </div>
  );
}
